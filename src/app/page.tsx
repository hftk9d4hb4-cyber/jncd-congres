...
import { ArrowRight, CalendarDays, MapPin, Stethoscope, Users, ShieldCheck, ChevronRight } from "lucide-react";
import { Badge, Card, Container, LinkButton, SectionHeading } from "@/components/ui";
import { congress, programDays, speakers } from "@/data/site";
...
export default function HomePage() {
  return <>
    ...
    <section className="bg-slate-50 py-20 sm:py-28"><Container><div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between"><SectionHeading eyebrow="Programme provisoire" title="Deux journées pour confronter les pratiques" description="Le programme sera enrichi progressivement avec les noms des intervenants, les modérateurs et les objectifs pédagogiques de chaque session." /><Link href="/programme" className="inline-flex items-center gap-2 text-sm font-semibold text-sky-800">Programme complet <ChevronRight size={17} /></Link></div><div className="mt-12 grid gap-5 lg:grid-cols-2">{programDays.map((day) => <Card key={day.id} className="p-7"><p className="text-xs font-bold uppercase tracking-[0.2em] text-sky-700">{day.label}</p><h3 className="mt-3 text-2xl font-semibold text-slate-950">{day.eyebrow}</h3><div className="mt-6 space-y-4">{day.items.slice(1,5).map((item) => <div key={`${item.time}-${item.title}`} className="grid grid-cols-[56px_1fr] gap-3 border-t border-slate-200 pt-4"><span className="font-mono text-xs font-semibold text-slate-500">{item.time}</span><span className="text-sm font-medium text-slate-800">{item.title}</span></div>)}</div></Card>)}</div></Container></section>

    <section className="py-20 sm:py-28"><Container><SectionHeading eyebrow="Intervenants" title="Une programmation nationale et internationale" description="Les fiches ci-dessous illustrent la structure prévue. Les invités et leurs interventions seront confirmés dans une version ultérieure." /><div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{speakers.slice(0,3).map((speaker) => <Card key={speaker.name} className="p-6"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-slate-950 text-sm font-bold text-white">{speaker.initials}</div><h3 className="mt-5 font-semibold text-slate-950">{speaker.name}</h3><p className="mt-1 text-sm text-sky-700">{speaker.role}</p><p className="mt-3 text-sm text-slate-500">{speaker.institution}</p><p className="mt-5 border-t border-slate-200 pt-4 text-sm font-medium text-slate-700">{speaker.topic}</p></Card>)}</div><div className="mt-8"><LinkButton href="/intervenants" variant="secondary">Voir les intervenants</LinkButton></div></Container></section>
